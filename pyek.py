...
def __menu__():
	...
	print("[%s02%s]%s Dump id publik new (2020-2021)"%(H,B,P))
	print("[%s03%s]%s Dump id publik acak (2004-2021)"%(H,B,P))
	print("[%s04%s]%s Dump id publik masal new (2020-2021)"%(H,B,P))
	print("[%s05%s]%s Dump id publik masal acak (2003-2021)"%(H,B,P))
	print("[%s06%s]%s Mulai crack"%(K,B,H))
	print("[%s07%s]%s Lihat hasil crack"%(H,B,P))
	print("[%s08%s]%s Hapus tokenz"%(M,B,K))
	__menu=raw_input("\n%s«%s?%s Choose :%s "%(B,H,P,X))
	if __menu in ['']:
		exit("%s«%s!%s Wrong input"%(K,M,K))
	elif __menu in ['1','01']:
